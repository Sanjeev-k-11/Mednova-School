...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['delete_program'])) {
        $program_id = $_POST['program_id']; // FIX: Was club_id
    ...
    // --- CREATE / UPDATE ACTION ---
        ...
        $program_id = $_POST['program_id'] ?? null; // FIX: Was club_id
        $name = trim($_POST['name']);
        $description = trim($_POST['description']);
        $teacher_id = !empty($_POST['teacher_in_charge_id']) ? trim($_POST['teacher_in_charge_id']) : null;
        $schedule = trim($_POST['program_date']); // NOTE: variable name was schedule, but field is program_date
        ...
        if (empty($name) || empty($status) || empty($schedule)) {
            $_SESSION['error_message'] = "Program Name, Date, and Status are required.";
        } else {
            if ($program_id) { // UPDATE. FIX: was club_id
                ...
                if ($stmt = mysqli_prepare($link, $sql)) {
                    mysqli_stmt_bind_param($stmt, "ssisssisis", $name, $description, $teacher_id, $schedule, $location, $image_url, $reg_deadline, $limit, $status, $program_id);
                ...
                if ($stmt = mysqli_prepare($link, $sql)) {
                    mysqli_stmt_bind_param($stmt, "ssisssiss", $name, $description, $teacher_id, $schedule, $location, $image_url, $reg_deadline, $limit, $status);
...
<div class="container mx-auto mt-28 max-w-7xl p-4 sm:p-6">
    ...
    <!-- The rest of your HTML is correct and does not need to be changed. -->
...
<div id="programModal" class="fixed z-50 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen px-4"><div class="fixed inset-0 bg-black bg-opacity-50" onclick="closeModal('programModal')"></div>
        <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-3xl w-full z-10">
            <form id="programForm" action="admin_cultural_programs.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="program_id" id="program_id"><input type="hidden" name="current_image_url" id="current_image_url">
                <div class="px-6 py-4 border-b"><h3 id="modalTitle" class="text-xl font-bold text-gray-900">Create New Program</h3></div>
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div><label for="name" class="block text-sm font-medium text-gray-700">Program Name</label><input type="text" name="name" id="name" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg" required></div>
                    ...
                    <div><label for="program_date" class="block text-sm font-medium text-gray-700">Program Date</label><input type="date" name="program_date" id="program_date" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg" required></div>
...
<script>
    ...
    function confirmDelete(id, name) { 
        if (confirm(`Are you sure you want to delete "${name}"?`)) { 
            ...
            form.innerHTML = `<input type="hidden" name="program_id" value="${id}"><input type="hidden" name="delete_program" value="1">`; 
