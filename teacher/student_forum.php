...
if (!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true || !in_array($_SESSION['role'], [ 'Teacher'])) {
...
if ($user_role === 'Student') {
    ...
    if ($user_class_id) {
        ...
        if (!$is_authorized) {
            // Don't die, just show an error message within the page layout
...
if ($user_class_id) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['create_post'])) {
        ...
        header("location: student_forum.php?class_id=" . $user_class_id);
        exit;
    }
